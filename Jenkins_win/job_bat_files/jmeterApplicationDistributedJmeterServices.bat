...
for %%s in ("%slaveServers:,=" "%") do (
	...
	for /f "tokens=1,2 delims=:" %%a in (%%s) do (
		echo   RUNNING COMMAND: /node:%%a process call create "%JmeterHome%\bin\jmeter-server.bat -Djava.rmi.server.hostname=%%b"
		wmic /node:%%a process call create "%JmeterHome%\bin\jmeter-server.bat -Djava.rmi.server.hostname=%%b"
...
echo    RUNNING COMMAND:  wmic process call create "%JmeterHome%\bin\jmeter-server.bat -Djava.rmi.server.hostname=%MasterIP%"
wmic process call create "%JmeterHome%\bin\jmeter-server.bat -Djava.rmi.server.hostname=%MasterIP%"
