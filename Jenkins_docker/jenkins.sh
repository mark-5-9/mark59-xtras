...
if [[ $# -lt 1 ]] || [[ "$1" == "--"* ]]; then
  ...
  echo "--- Setting hudson.model.DirectoryBrowserSupport.CSP empty"
  exec java -Duser.home="$JENKINS_HOME" "${java_opts_array[@]}"  -Dhudson.model.DirectoryBrowserSupport.CSP= -jar ${JENKINS_WAR} "${jenkins_opts_array[@]}" "$@"
fi
